...
//http://open.youtu.qq.com/#/develop/api-face-identify-multiidentify
...
public enum Template
{
    ...
    cf_yuren_smart,
    [Header("南昌起义男")]
    hezuo_junzhuangzhao_1927m_20170919140703,
    [Header("南昌起义女")]
    hezuo_junzhuangzhao_1927w2_20170919140735,
    [Header("抗日战争男")]
    hezuo_junzhuangzhao_1937m_20170919140902,
    [Header("抗日战争女")]
    hezuo_junzhuangzhao_1937w_20170919140927,
    [Header("解放战争男")]
    hezuo_junzhuangzhao_1948m_20170919140951,
    [Header("解放战争女")]
    hezuo_junzhuangzhao_1948w_20170919141013,
    [Header("50式军服男")]
    hezuo_junzhuangzhao_1950m_20170919141035,
    [Header("50式军服女")]
    hezuo_junzhuangzhao_1950w_20170919141055,
    [Header("65式军服男")]
    hezuo_junzhuangzhao_1965m_20170919141309,
    [Header("65式军服女")]
    hezuo_junzhuangzhao_1965w_20170919141337,
    [Header("87式军服男")]
    hezuo_junzhuangzhao_1987m_20170919141505,
    [Header("87式军服女")]
    hezuo_junzhuangzhao_1987w_20170919141529,
    [Header("99式军服男")]
    hezuo_junzhuangzhao_1999m_20170919141604,
    [Header("99式军服女")]
    hezuo_junzhuangzhao_1999w_20170919141627,
    [Header("07式军服男")]
    hezuo_junzhuangzhao_2007m_20170919141654,
    [Header("07式军服女")]
    hezuo_junzhuangzhao_2007w_20170919141715
}
...
public class Youtu
{
    ...
    /// 人脸检测 detectface
    /// </summary>
    /// <param name="image_byte">的字节</param>
    /// <param name="isbigface">是否大脸模式 ０表示检测所有人脸， 1表示只检测照片最大人脸　适合单人照模式</param>
    /// <returns>返回的结果，JSON字符串，字段参见API文档</returns>
    public static string detectface(byte[] image_byte,int isbigface= 1)
    {
        string expired = Utility.UnixTime(EXPIRED_SECONDS);
        string methodName = "youtu/api/detectface";
        StringBuilder postData = new StringBuilder();
        string pars = "\"app_id\":\"{0}\",\"image\":\"{1}\",\"mode\":{2}";
        pars = string.Format(pars, Conf.Instance().APPID, Utility.ByteArrToStr64(image_byte), isbigface);
        postData.Append("{");
        postData.Append(pars);
        postData.Append("}");
        string result = Http.HttpPost(methodName, postData.ToString(), Auth.appSign(expired, Conf.Instance().USER_ID));
    ...
    /// <param name="isbigface">是否大脸模式 ０表示检测所有人脸， 1表示只检测照片最大人脸　适合单人照模式</param>
    /// <returns>返回的结果，JSON字符串，字段参见API文档</returns>
    public static string faceshape(byte[] image_byte, int isbigface = 1)
    {
        string expired = Utility.UnixTime(EXPIRED_SECONDS);
        string methodName = "youtu/api/faceshape";
        StringBuilder postData = new StringBuilder();
    ...
    public static string faceMerge(byte[] image_byte, string rsp_img_type, string opdata)
    {
        string expired = Utility.UnixTime(EXPIRED_SECONDS);
        string methodName = "cgi-bin/pitu_open_access_for_youtu.fcg";
        StringBuilder postData = new StringBuilder();
        ...
        string pars = "\"img_data\":\"{0}\",\"rsp_img_type\":\"{1}\",\"opdata\":{2}";
        pars = string.Format(pars, Utility.ByteArrToStr64(image_byte), rsp_img_type, opdata);
        postData.Append("{");
        postData.Append(pars);
        postData.Append("}");
        string result = Http.HttpPost(methodName, postData.ToString(), Auth.appSign(expired, Conf.Instance().USER_ID));
